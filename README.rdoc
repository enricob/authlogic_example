...
3. A particular LDAP schema is assumed, which is as below:
  ...
  * <tt>dc=example,dc=com</tt> (domain)
   ...
   The +LdapUser+ class encapsulates LDAP user entries and can be changed to match your own schema (see the ActiveLdap docs).
   Also see the +ldap.yml.example+ file to configure your domain and users prefix.
4. ruby-net-ldap 0.0.5 (built from GitHub) is used because 0.0.4 from RubyForge has a bug which prevents DN's from being used
